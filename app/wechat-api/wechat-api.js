...
var semanticPrefix = 'https://api.weixin.qq.com/semantic/';
var oauth2Prefix = 'https://api.weixin.qq.com/sns/oauth2/';
...
var api ={
    ...
    permanent: {//永久素材管理
        upload: prefix + 'material/add_material?',//上传添加
        fetch: prefix + 'material/get_material?',//获取
        del: prefix + 'material/del_material?',//删除
        update: prefix + 'material/update_material?',//更新
        count: prefix + 'material/get_materialcount?',//数量
        batch: prefix + 'material/batchget_material?',//批量获取
    ...
    semantic:{
        search: semanticPrefix + 'search?'
    },
    ticket:{//JSSDK凭证获取
        get: prefix + 'ticket/getticket?'
    },
    oauth2:{//微信网页授权
        getAccessToken: oauth2Prefix + 'access_token?'
    },
    template:{//模板消息
...
function Wechat(opts){
    ...
    console.log('创建wechat所需appSecret，appID：', opts.appSecret, opts.appID);
...
Wechat.prototype.fetchAccessToken = function(){
    var that = this;
    console.log('access_token', this.access_token, this.expires_in);
...
Wechat.prototype.updateAccessToken = function(){
    ...
    console.log('更新accessToken的appID，appSecret', appID, appSecret);
    ...
    return new Promise(function(resolve, reject){
        ...
        request({url: url,json: true}).then(function(response){
            ...
            console.log('最新access_token', data.access_token, data.expires_in);
        ...
        that.fetchAccessToken().then(function(data){
            ...
            request({method:'POST', url: url, body: form, json: true}).then(function(response){
                ...
                if(_data){
                    ...
                    throw new Error('Del Group fails');
                }
        ...
        that.fetchAccessToken().then(function(data){
            ...
            request({method:'POST', url: url, body: msg, json: true}).then(function(response){
                ...
                if(_data){
                    ...
                    throw new Error('Send to grounp fails');
                }
...
Wechat.prototype.semantic = function(sData){
    var that = this;

    return new Promise(function(resolve, reject){
        that.fetchAccessToken().then(function(data){
            var url = api.semantic.search +'access_token='+data.access_token;
            sData.appid = data.appID;

            request({method:'POST', url: url, body: sData, json: true}).then(function(response){
                var _data = response['body'];
...
Wechat.prototype.getOpenId = function(appId, secret, code){
    var that = this;

    return new Promise(function(resolve, reject){
        var url = api.oauth2.getAccessToken + `appid=${appId}&secret=${secret}&code=${code}&grant_type=authorization_code`;
        console.log('获取网页授权access_token请求地址：', url);
        ...
        request({url: url, json: true}).then(function(response){
            ...
            if(_data){
                ...
                throw new Error('getOpenId fails');
            }
...
//发送模板消息
Wechat.prototype.sendTemplate = function(sData){
    var that = this;
        ...
        that.fetchAccessToken().then(function(data){
            ...
            request({method:'POST', url: url, body: sData, json: true}).then(function(response){
                ...
                if(_data){
                    ...
                    throw new Error('sendTemplate fails');
                }
